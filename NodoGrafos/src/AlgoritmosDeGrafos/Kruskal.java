...
public class Kruskal {
	...
	static class Edge implements Comparator<Edge> {
		...
		// orden ascendente
		// Ordenar de forma descendente para obtener el arbol de expansion
		// maxima
		@Override
	...
	static void KruskalMST() {
		...
		for (int i = 0; i < E; ++i) { // Recorremos las aristas ya ordenadas por
										// peso
		...
		// error
		// Para saber si contiene o no todos los vértices basta con que el
		// numero
		// de aristas sea igual al numero de vertices - 1
		if (V - 1 != numAristas) {
			System.out
					.println("No existe MST valido para el grafo ingresado, el grafo debe ser conexo.");
			return;
		}
		System.out
				.println("-----El MST encontrado contiene las siguientes aristas-----");
		for (int i = 0; i < numAristas; ++i)
			System.out.printf("( %d , %d ) : %d\n", MST[i].origen,
					MST[i].destino, MST[i].peso); // ( vertice u , vertice v ) :
													// peso
	...
	public static void main(String[] args) {
		...
		V = sc.nextInt();
		E = sc.nextInt();
		// Realizamos el ingreso del grafo, almacenando las aristas en un
		// arreglo con los datos respectivos
		for (int i = 0; i < E; ++i) {
			arista[i] = new Edge();
